...
void copy_completion_irq(uint8_t copy_type) {
	// stub for copy completion IRQ
}
...
void request_read(uint64_t dst, uint64_t src, uint64_t n) {
	launch_copy_thread(dst, src, n, READ_FROM_DEVICE, &ongoing_read, ram + dst, vram + src);
}

void request_write(uint64_t dst, uint64_t src, uint64_t n) {
	launch_copy_thread(dst, src, n, WRITE_TO_DEVICE, &ongoing_write, vram + dst, ram + src);
}
